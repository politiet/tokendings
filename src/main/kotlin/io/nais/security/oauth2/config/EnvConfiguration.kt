...
import io.nais.security.oauth2.model.IssuerSubjectTokenMappings
...
object Configuration {
    ...
    private val issuerSubjectTokenMappings: Map<String, List<SubjectTokenMapping>> = konfig.getOrNull(Key(SUBJECT_TOKEN_MAPPINGS, stringType))?.let {
        IssuerSubjectTokenMappings.fromJson(it)
    } ?: emptyMap()
    val instance by lazy {
        val databaseConfig = migrate(databaseConfig())
        val authorizationServerProperties = AuthorizationServerProperties(
            issuerUrl = issuerUrl,
            subjectTokenIssuers = subjectTokenIssuers.toConfiguration(issuerSubjectTokenMappings),
...
fun List<String>.toConfiguration(issuerSubjectTokenMappings: Map<String, List<SubjectTokenMapping>>) = this
    .map { issuerWellKnown ->
        SubjectTokenIssuer(issuerWellKnown, issuerSubjectTokenMappings[issuerWellKnown] ?: emptyList())
    }
